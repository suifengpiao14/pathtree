...
{{define "ddlCarAttribute"}}
    create table  if not exists `car_attribute`(
        attr_key varchar(64) not null default "" comment "外部节点标识",
        title varchar(64) not null default "" comment "标题",
        parent_id varchar(64) not null default "" comment "父节点ID",
        `order` tinyint(1) not null  default 0 comment "序列号(兄弟节点排序)",
        is_leaf enum("YES","NO")  default "NO" comment "是否为叶子节点(YES-是,NO-否)",
    ...
    )ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8mb4 comment "汽车属性模型";
{{end}}